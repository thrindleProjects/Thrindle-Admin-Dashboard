...
import PasswordInput from "./CreateModal/PasswordInput";

const UnPCreateModal = () => {
  const handleFormSubmit = (e) => {
    e.preventDefault();
  };

  return (
    <CreateModal className="h-fit px-6 py-9 text-primary-main flex flex-col gap-6 justify-center">
      ...
      <form onSubmit={handleFormSubmit} className={`flex flex-col gap-5`}>
        ...
        <div className="w-full flex justify-end gap-4">
          ...
          <ButtonWrapper className="approve">Approve</ButtonWrapper>
        </div>
      </form>
    </CreateModal>
  );
