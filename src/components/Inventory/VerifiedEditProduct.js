...
const VerifiedEditModal = (props) => {
    ...
    async (id) => {
      ...
      try {
        ...
        setFormData({
          ...
          subcategory,
          weight,
          activeImage: `https://store-staging-api.thrindle.com/api/thrindle/images/${images[0]}`,
          details: {
            size,
            color,
          },
  ...
  const handleImageDelete = (e, url) => {
    ...
    if (activeImage === url) {
      setFormData((old) => ({
        ...old,
        activeImage: `https://store-staging-api.thrindle.com/api/thrindle/images/${newImages[0]}`,
      }));
    }
