...
function MerchantData() {
	...
	const heading2 = ["S/N", "Buyer's Name", "Buyer's Email", "Date", "Action"];
	...
	return (
      ...
      <MerchantHeader text="Merchant's Data" />
          ...
          <div className='px-6 '>
            <p className='my-4 text-sm '>Total Earnings : N {data?.TotalEarnings}</p>
					<p className='my-4 text-sm '>Balance: N {data?.WalletBalance}</p>
          </div>
          <div>
            {data.WalletTransactions.length !== 0 && <div className=" bg-white  px-6 py-10 border rounded-lg">
          ...
          {/* store orders */}
          {data.StoreOrders.length !==0 && <><h1 className='my-6 px-6 text-brand-color'>Store Orders</h1>
					<div>
						<div className=" bg-white  px-6 py-10 border rounded-lg">
							<table className="table-auto w-full   ">
								...
								<tbody>
									{data?.StoreOrders?.map((item, index) => (
										<tr
											key={index}
